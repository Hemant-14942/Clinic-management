...
const AddDoctor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!formData.image) return toast.error("Image not selected");
      if (!address1 || !address2) return toast.error("Both address lines are required");
      if (
        !formData.name ||
        !formData.specialty ||
        !formData.email ||
      ...
      const addressData = { line1: address1, line2: address2 };
  ...
  const selectClasses =
    "w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/30 focus:border-primary/30 transition-all duration-200 outline-none bg-white";
    ...
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="pt-4">
          <button
            type="submit"
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-6 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 flex items-center justify-center"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5 mr-2"
